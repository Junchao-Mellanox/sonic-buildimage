...
class ContainerConfigDaemon(daemon_base.DaemonBase):
    ...
    def run(self):
        ...
        config_db.listen(init_data_handler=self.load)
...
class SyslogHandler:
    ...
    def update_syslog_config(self, data):
        ...
        Args:
            data (dict): Data of the entry: {<field_name>: <field_value>}
        """
        cur_interval, cur_burst, target_ip = self.parse_syslog_conf()
        ...
        if new_interval == cur_interval and new_burst == cur_burst:
            return
        ...
        run_command('sonic-cfggen -d -t /usr/share/sonic/templates/rsyslog-container.conf.j2 -a "{{\\"target_ip\\": \\"{}\\", \\"container_name\\": \\"{}\\" }}"  > {}'.format(target_ip, container_name, self.TMP_SYSLOG_CONF_PATH))
